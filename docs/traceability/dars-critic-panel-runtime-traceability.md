...
## DARS-LIVE-RELEASE-R4-CODEX-SUBPROCESS-PANEL-SMOKE-AUTH-STOP — Attempted live panel smoke blocked by Codex auth (2026-05-24)

- Scope: after explicit approval for `R4 Codex subprocess panel smoke`, Hisys attempted the governed Codex CLI subprocess prompt-mode panel path and stopped on Codex CLI auth refresh failure before critique output.
...
- Traceability: HISYS-FR-DARS-CP-012 / HISYS-T-DARS-CP-014 is now `LIVE-ACTION-ATTEMPTED + AUTH-BLOCKED + HUMAN-AUTH-RECOVERY-REQUIRED` for the R4 mapped-subscription panel path.
- Boundary: Codex CLI attempted its own subscription auth refresh and failed. Hisys did not inspect credentials, call a raw provider API, produce critique output, write panel evidence, mutate, publish, deploy, release, execute R5/R7/R8, or remove human review.
