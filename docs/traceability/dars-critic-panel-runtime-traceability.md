...
| HISYS-FR-DARS-CP-015 | release checklist, release notes, release scope decision packet, RC readiness decision packet, residual-risk human gate, post-release smoke evidence | HISYS-T-DARS-CP-017 | `test_release_candidate_checklist_requires_prior_live_unattended_and_rollback_evidence`; `test_rc_scope_decision_preserves_claim_ladder_and_blocks_release_actions`; `test_rc_readiness_packet_records_no_go_claim_boundary`; `test_residual_risk_gate_records_gate_entered_without_approval_substitution`; release decision packet review | RESIDUAL-RISK-GATE-ENTERED + EXACT APPROVAL REQUIRED (DARS-LIVE-RELEASE-R7; `release_candidate_ready=false`) |
