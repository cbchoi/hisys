...
|---|---|---|---|---|
| HISYS-FR-DARS-CP-001 | `DarsCriticPanelConfig`, config validator, `CriticAdapterRegistry` (M-CP-EXT-1), read-only `hisys run-dars-panel` CLI wrapper (M-CP-EXT-6), activation-packet-gated local-model CLI rehearsal (M-CP-LIVE-3) | HISYS-T-DARS-CP-001 | `test_dars_critic_panel_config_validates_two_advisory_roles`, `test_critic_adapter_registry_rejects_duplicate_role_backend_pair`, `test_run_dars_panel_cli_persists_fixture_round_and_prints_json`, `test_run_dars_panel_cli_requires_activation_packet_for_local_model_mode`, `test_run_dars_panel_cli_rehearses_local_model_with_activation_packet` | GREEN (MB-DARS-CP-T001 + M-CP-EXT-1 + M-CP-EXT-6 + M-CP-LIVE-3) |
