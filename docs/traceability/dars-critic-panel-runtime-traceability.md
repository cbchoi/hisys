...
- Scope: after the v0.0.109 execution decision packet, the operator instructed `go`, then later instructed `execute`. At the scoped execution instruction gate, neither generic `go` nor unscoped `execute` is a package-upload execution instruction. This records the missing scoped instruction and keeps the gate open.
