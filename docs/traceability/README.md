...
| Live-U Queue admission validator | docs/use-cases/live-research-connectors.md; docs/use-cases/obsidian-live-research-layout.md; HISYS-FR-INV-001..006; HISYS-FR-MEM-001..005; HISYS-T-024; HISYS-CON-010..012; HISYS-CON-022..023 | Implemented: `hisys live-autonomy-admit` validates candidate queue JSON files before scheduler intake, admits valid queues to `--incoming-dir`, rejects malformed or schema-incomplete queues to `--rejected-dir`, writes deterministic admission reports with no external call, vault mutation, or network push, and keeps scheduler overhead limited to local JSON/filesystem checks. |
