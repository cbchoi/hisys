...
| Live-X Duplicate/replay classification | docs/use-cases/live-research-connectors.md; docs/use-cases/obsidian-live-research-layout.md; HISYS-FR-INV-001..006; HISYS-FR-MEM-001..005; HISYS-T-024; HISYS-CON-010..012; HISYS-CON-022..023 | Implemented: `hisys live-autonomy-admit` compares candidate queue and entry hashes against existing incoming/rejected handoff artifacts and records `new`, `same_hash_replay`, `changed_same_entry_id`, or `duplicate_queue_content` classifications as deterministic report-only metadata with no external call, vault mutation, Git/network push, or routine prompt check. |
