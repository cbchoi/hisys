...
| I5 Extraction foundation | HISYS-IMP-001 Section 3; HISYS-SCHEMA-001 Section 5; HISYS-FR-EXT-001..005 | Fixture-backed `RawObservation` -> `ExtractedSignal` extractor and local signal JSON persistence |
...
Full workflow coverage remains pending; later increments (I6 editorial,
...
- HISYS-T-009..010: Extraction foundation creates `ExtractedSignal` records
  from `RawObservation` evidence references, preserves confidence/uncertainty/
  contradiction metadata, avoids raw payload copying, and persists signal JSON
  under the runtime instance.
