...
| I8-A/B/C DARS advisory handoff loopback and config-validation contract | HISYS-IMP-001 Section 3; HISYS-SCHEMA-001 Section 9; HISYS-FR-AGT-001..005; HISYS-DARS-CONTRACT-001; HISYS-T-019; HISYS-T-020; HISYS-CON-010..012 | Runtime-local `hisys request-dars-critique` creates advisory-only `AgentHandoffPackage` JSON/Markdown records linked to disabled connector executions and returns loopback placeholder critique records by default because DARS is not implemented yet; artifacts record `dars_backend=loopback_placeholder`, `external_call_made=false`, `allowed_actions=advisory_only`, `action_taken=none`, and no live DARS call or external action occurs. `hisys.config.validation` and `hisys.agents.dars_config` validate common config envelopes plus DARS role/backend specs before any DARS adapter can run. |
...
safety gates before any live adapter can be added. I8-A/B/C adds runtime-local advisory DARS handoff loopback artifacts without
implementing DARS or making live DARS calls, plus a common configuration
envelope validator and DARS backend/role config contract that keeps non-loopback
backends disabled by default. Investigator multi-agent fixture
