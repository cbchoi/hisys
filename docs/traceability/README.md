...
| I8-A/B/C/D DARS advisory handoff loopback, config-validation contract, and data-format contract | HISYS-IMP-001 Section 3; HISYS-SCHEMA-001 Section 9; HISYS-FR-AGT-001..005; HISYS-DARS-CONTRACT-001; HISYS-T-019; HISYS-T-020; HISYS-T-024; HISYS-CON-010..012 | Runtime-local `hisys request-dars-critique` creates advisory-only `AgentHandoffPackage` JSON/Markdown records linked to disabled connector executions and returns loopback placeholder critique records by default because DARS is not implemented yet; artifacts record `dars_backend=loopback_placeholder`, `external_call_made=false`, `allowed_actions=advisory_only`, `action_taken=none`, and no live DARS call or external action occurs. `hisys.config.validation` and `hisys.agents.dars_config` validate common JSON config envelopes plus DARS role/backend specs from `config/dars.json` before any DARS adapter can run. `docs/contracts/dars-data-format.md` defines canonical `DarsRequestEnvelope` and `DarsResponseEnvelope` boundary artifacts plus progressive adversarial decision-trace fields for future DARS protocol models. |
