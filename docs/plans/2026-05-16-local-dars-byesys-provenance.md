...
5. **Terminology normalization**
   ...
   - Keep **Devil** terminology, with **DARS Devil** / **DARS reviewer** retained as explicit aliases for advisory critique separation; do not migrate DARS to Appraiser.
   - Preserve legacy Python package/class names temporarily where migration would be too large, but add user-facing aliases and documentation so future generated docs and prompts use Jeweler for final review and Devil for adversarial critique consistently.
   - Add a terminology map: `Chief Editor -> Jeweler`; `DARS devil -> Devil`; `DARS reviewer -> Devil`; `Appraiser -> Devil` for any interim drafts that used Appraiser.
...
- Jeweler/Devil alias map exists and is documented; Appraiser is treated only as a deprecated draft alias for Devil.
