...
The active authoritative `/rloo` queue is this `ralph.md` file. The current branch is `/home/cbchoi/workspaces/develop/repos/hisys` on `dars`. M22 codebase evidence portfolio milestone is closed at `local_fixture_advisory_complete` (`cd944cc`). The user has explicitly authorized M23 advanced codebase adapter integration, including approved OSS comparison adapter work and optional local LSP adapter work; the OSS comparison adapter has now landed under the governed M23 boundary recorded below. The `M23-OSS-ADAPTER-RED-GREEN` row implemented the pure builder/writer at `src/hisys/operations/oss_comparison_adapter.py` plus six focused tests in `tests/unit/test_oss_comparison_adapter.py`, so the next safe Ralph row is the thin CLI wrapper.
...
Done: M23-OSS-ADAPTER-RED-GREEN — pure approved-OSS comparison adapter, writer, and six focused tests landed under the M23 authorization boundary; combined M21+M22+M23 focused gate `72 passed`, DARS panel focused regression `55 passed`.
Next: M23-OSS-ADAPTER-CLI — author docs/plans/m23-cli-oss-comparison-adapter-implementation-tasks.md and add a thin `hisys oss-comparison-adapter` argparse subparser that loads a caller-supplied JSON bundle and delegates to the pure builder/writer; no `date.today()`, no `.git/` read, no `subprocess`, no `tests/fixtures/` crawl, no `runtime-boundary/` crawl, no network, no OSS clone/install, no auto-discovery.
Pending after OSS CLI: M23-OSS-ADAPTER-GOLDEN, M23-OSS-ADAPTER-GATE, M23-LSP-ADAPTER-PREP, M23-LSP-ADAPTER-RED-GREEN, M23-ADAPTER-PORTFOLIO-INTEGRATION, M23-ADVANCED-ADAPTER-GATE.
```

Next safe Ralph queue target:

```text
M23-OSS-ADAPTER-CLI — author the CLI PREP packet `docs/plans/m23-cli-oss-comparison-adapter-implementation-tasks.md` documenting the JSON-bundle pass-through decision, then add four focused CLI tests to `tests/unit/test_domain_cli.py` (happy-path round-trip, missing line bundle rejection, bad-date rejection, and unsafe-input recording) and implement the subparser + bundle loader + `_cmd_oss_comparison_adapter` dispatcher in `src/hisys/cli/main.py`. The CLI must construct `LocalCodebaseLine`/`ApprovedOssSource`/`OssComparisonRequest` through the existing Pydantic models, must reuse the existing JSON loader helper, must print bounded summary lines (`oss-comparison-adapter report`, `markdown`, `compared_source_count`, `union_category_count`, `intersection_category_count`, `local_only_category_count`, `oss_only_category_count`, `unsafe_ref_count`, `unsafe_source_id_count`, `unsafe_line_label_count`, `advisory_only: true`, `requires_human_review: true`, `external_call_made: false`, `mutation_performed: false`, `raw_source_content_persisted: false`, `live_external_action_authorized: false`, `allowed_actions: advisory_only`), must return exit code `0` on success and propagate `ValueError` non-zero, and must never call `date.today()`, must never read `.git/`, must never call `subprocess`, must never crawl `tests/fixtures/` or `runtime-boundary/`, must never auto-discover bundle files, must never open `local_fixture_refs` to verify file existence, must never install OSS packages, and must never expand the source-id/category/line-label vocabulary. Update `docs/traceability/README.md`, `docs/milestone-bootstrap/profile.yaml` (next_safe_task → `M23-OSS-ADAPTER-GOLDEN`), and `tests/unit/test_governance_docs_current_state.py` (v0.0.24).
```
