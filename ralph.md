...
Next: M23-LSP-ADAPTER-PREP — author docs/plans/m23-lsp-adapter-implementation-tasks.md recording the local LSP adapter subprocess boundary (command allowlist, timeout, workspace-root restriction, output schema, kill policy). Docs/control only; no subprocess spawns from PREP.
Pending after LSP PREP: M23-LSP-ADAPTER-RED-GREEN (separately governed subprocess work), M23-ADAPTER-PORTFOLIO-INTEGRATION (optional additive evidence-line row), M23-ADVANCED-ADAPTER-GATE (closure).
```

Next safe Ralph queue target:

```text
M23-LSP-ADAPTER-PREP — author `docs/plans/m23-lsp-adapter-implementation-tasks.md` recording the local LSP adapter subprocess boundary BEFORE any product code. The PREP packet must define: (1) the LSP server command allowlist (which binaries may be spawned and from which absolute paths), (2) the subprocess timeout policy (per-call and aggregate), (3) the workspace-root restriction that prevents the LSP from reading outside the caller-supplied root, (4) the LSP output schema (deterministic JSON only, no raw source bodies, bounded category/ref strings), (5) the kill policy (signal sequence, escalation, and stale-process cleanup), (6) the first RED test command and expected ModuleNotFoundError, (7) focused and regression gate commands, and (8) traceability anchors. Do not implement product code or spawn any subprocess from the PREP row itself. After PREP commits, `M23-LSP-ADAPTER-RED-GREEN` may run only with explicit user re-authorization because the subprocess spawn falls under the separately-governed boundary.
```
