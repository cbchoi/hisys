...
| Active task | `DARS-LIVE-RELEASE-R5-UNATTENDED-PREP` (R1+R2 GREEN; R3 PREP GREEN; R4 PREP GREEN; R5 PREP prepares the bounded standing-approval policy and unattended runner contract with dry-run rehearsal evidence. R3/R4/R5 ACTION rows remain HUMAN-GATED.) |
...
R1, R2, R3 PREP, and R4 PREP are GREEN. The next executable task is R5 PREP: implement the bounded standing-approval policy validator, the unattended advisory runner with audit ledger and circuit breakers, and dry-run rehearsal evidence with fake/injected transports. R5 PREP is local Python + tests + docs and remains within the local-safe authorization envelope; the bounded unattended live canary (R5 ACTION) is a separately approved HUMAN-GATED action.
...
### Next implementation: `DARS-LIVE-RELEASE-R5-UNATTENDED-PREP`

Objective: implement the bounded standing-approval policy validator and the unattended advisory runner contract with dry-run/fake-transport rehearsal evidence. R5 PREP is local Python + tests + docs; the bounded unattended live canary (R5 ACTION) is a separately approved HUMAN-GATED action.

Remaining expected files (PREP scope):

- `src/hisys/agents/dars_unattended_policy.py` (new) — `StandingApprovalPolicy` validator that enforces expiry, request-class allowlists, budget/rate caps, kill-switch ref, audit-retention ref, `requires_post_run_human_review=true`, `mutation_allowed=false`, `publication_allowed=false`, `external_action_allowed=false`, and rejects raw secret fields.
- `src/hisys/operations/dars_unattended_runner.py` (new) — `DarsUnattendedAdvisoryRunner` that executes governed advisory requests through the R2 adapter and writes an audit ledger entry per run.
- `tests/unit/test_dars_unattended_policy.py` (new) — RED-first tests for expiry, missing kill-switch, missing budget caps, mutation/publication/action authority rejection, raw-secret rejection, and `requires_post_run_human_review=true` invariant.
- `tests/unit/test_dars_unattended_runner.py` (new) — RED-first tests for runner refusal when policy expired / kill-switch missing / budget caps absent / any mutation authority requested; GREEN tests for dry-run fake-transport execution and audit ledger writing; GREEN tests for circuit breakers (repeated failures, cost threshold, secret scan hit, policy mismatch, output redaction failure).
- `docs/runbooks/dars-unattended-advisory-operation.md` (done in docs checkpoint) — PREP scope: standing approval policy contract, runner procedure, dry-run rehearsal procedure, circuit breaker matrix, post-run human review.
- `docs/examples/dars/unattended-standing-approval.example.json` (done in docs checkpoint) — sample standing approval policy for the future validator.
- updates to `docs/traceability/dars-critic-panel-runtime-traceability.md` and ralph.md (docs checkpoint done; final R5 PREP GREEN update remains after implementation).

GREEN requirements for R5 PREP:

- standing approval policy validator + RED→GREEN tests pass;
- unattended runner + RED→GREEN tests pass with fake/injected transports only;
- dry-run rehearsal records an audit ledger entry under the controlled instance root with no live call;
- runbook + example pass content checks and validator gates;
- secret scan + traceability validator continue to pass;
- ralph.md and RTM reflect R5 PREP GREEN and leave R5 ACTION (bounded unattended live canary) HUMAN-GATED.
