...
Done: M23-OSS-ADAPTER-GOLDEN — canonical bundle + checked-in expected JSON/Markdown + byte-equality round-trip test landed; OSS adapter focused gate `7 passed`.
Next: M23-OSS-ADAPTER-GATE — run the full focused + full repo-level gate sweep, append a `Done: M23-OSS-ADAPTER-GATE` line, perform QUEUE-REFILL-PREP classification, and decide whether the OSS adapter line closes at `local_fixture_advisory_complete` or whether `M23-ADAPTER-PORTFOLIO-INTEGRATION` should run first.
Pending after OSS GATE: M23-LSP-ADAPTER-PREP, M23-LSP-ADAPTER-RED-GREEN, M23-ADAPTER-PORTFOLIO-INTEGRATION, M23-ADVANCED-ADAPTER-GATE.
```

Next safe Ralph queue target:

```text
M23-OSS-ADAPTER-GATE — run the focused + full repo-level gate sweep (combined `tests/unit/test_oss_comparison_adapter.py tests/unit/test_domain_cli.py tests/unit/test_codebase_evidence_portfolio.py tests/unit/test_change_impact.py tests/unit/test_architecture_candidates.py tests/unit/test_code_analysis_pass_contract.py tests/unit/test_subagent_evidence_collector_protocol.py tests/unit/test_codebase_map_freshness.py tests/unit/test_runtime_boundary_consistency.py tests/unit/test_traceability_coverage.py tests/unit/test_governance_docs_current_state.py`, DARS critic-panel focused regression, full `PYTHONPATH=src pytest -q`, `python3 scripts/validate_traceability.py`, `python3 scripts/scan_secrets.py`, `git diff --check`). Append a `Done: M23-OSS-ADAPTER-GATE` line to ralph.md Section 16 with the gate evidence. Perform QUEUE-REFILL-PREP: if the only remaining backlog candidates are LSP adapter, portfolio integration, and the advanced gate, classify each as safe local docs/control vs. requires-separately-governed subprocess and either continue to the next safe row or stop with the explicit human-gated boundary. Update `docs/traceability/README.md`, `docs/milestone-bootstrap/profile.yaml` (next_safe_task → `M23-ADAPTER-PORTFOLIO-INTEGRATION` or `QUEUE-REFILL-PREP-STOP`), and `tests/unit/test_governance_docs_current_state.py`.
```
