...
The active authoritative `/rloo` queue is this `ralph.md` file. The current branch is `/home/cbchoi/workspaces/develop/repos/hisys` on `dars`. M22 codebase evidence portfolio milestone is closed at `local_fixture_advisory_complete` (`cd944cc`). The entire M23 advanced codebase adapter milestone is now closed at `local_fixture_advisory_complete`: OSS comparison adapter sub-line closed at `0a172d3` (full repo `999 passed`, OSS focused `7 passed`); LSP adapter sub-line closed at `3ca9b22` (full repo `1017 passed`, LSP focused `14 passed`, DARS panel `75 passed`, governance `1 passed`); M23-ADVANCED-ADAPTER-GATE consolidates both sub-lines into a single milestone record. Both adapters are pinned by deterministic byte-equality golden round-trip tests; the LSP runner is the first Hisys-managed local `subprocess` boundary and enforces the M23 subprocess safety contract (caller-supplied allowlist, 120s timeout cap, 4 MiB output truncation, workspace-root containment, no shell, no env inheritance beyond PATH, SHA-256 first-16-hex message_digest only — raw text never persisted). The next safe Ralph row is `QUEUE-REFILL-PREP-STOP` — all remaining post-M23 candidates (optional `M23-ADAPTER-PORTFOLIO-INTEGRATION`; live provider execution; real LSP binary invocation; OSS clone; M24 / new-milestone work) require fresh explicit user authorization beyond the existing M23 boundary.

Current queue status:

```text
Done: M22-PORTFOLIO-GATE — closed M22 at local_fixture_advisory_complete (cd944cc).
Done: M23-AUTH — user authorization captured in docs/milestone-bootstrap/documents/readiness_decision_record_v0.0.21.md and M23 parent plan created (9c095dd).
Done: M23-OSS-ADAPTER-PREP — docs/plans/m23-oss-comparison-adapter-implementation-tasks.md authored (70ae484).
Done: M23-OSS-ADAPTER-RED-GREEN — pure approved-OSS comparison adapter + 6 focused tests landed (d610c53).
Done: M23-OSS-ADAPTER-CLI — thin `hisys oss-comparison-adapter` CLI + 4 focused CLI tests landed (d6023b4).
Done: M23-OSS-ADAPTER-GOLDEN — canonical bundle + checked-in expected JSON/Markdown + byte-equality round-trip test landed (9963ccc).
Done: M23-OSS-ADAPTER-GATE — focused + full repo-level gate sweep confirmed `999 passed`; OSS adapter line closed at local_fixture_advisory_complete.
Done: M23-LSP-ADAPTER-PREP — docs/plans/m23-lsp-adapter-implementation-tasks.md authored with subprocess safety contract (caller-supplied command allowlist, timeout cap of 120s, 4 MiB output truncation, workspace-root containment, no shell, no environment inheritance beyond PATH, no raw message persistence — only SHA-256 first-16-hex digests, output_format allowlist of pyright_json/mypy_json/ruff_json/flake8_json/eslint_json, FileNotFoundError → ValueError("lsp_command_not_found")), the RED command and expected ModuleNotFoundError, the regression matrix, and the docs/gate/commit ritual.
Done: M23-LSP-ADAPTER-RED-GREEN — pure governed `src/hisys/operations/lsp_adapter.py` landed exposing `LspAdapterCommand`, `LspAdapterRequest`, `LspAdapterDiagnostic`, `LspAdapterReport`, `_validate_lsp_request`, five format-specific parsers (`_parse_ruff_json`, `_parse_pyright_json`, `_parse_mypy_json`, `_parse_flake8_json`, `_parse_eslint_json` — all `json.loads` only), `run_lsp_adapter`, `render_lsp_adapter_markdown`, and `write_lsp_adapter_report`. 13 focused tests in `tests/unit/test_lsp_adapter.py` pin the safety contract. No real LSP server is invoked; `subprocess.run` is monkey-patched in every test.
Done: M23-LSP-ADAPTER-CLI — thin `hisys lsp-adapter --instance <root> --date <YYYYMMDD> --bundle <json> [--current-head-short <hash>]` argparse subparser + `_load_lsp_adapter_bundle` loader + `_cmd_lsp_adapter` dispatcher landed inside `src/hisys/cli/main.py`, plus 4 focused CLI tests in `tests/unit/test_domain_cli.py`. The CLI re-imports the M23-RED-GREEN Pydantic models so it does not duplicate shape definitions; the pure runner re-validates every safety invariant.
Done: M23-LSP-ADAPTER-GOLDEN — canonical bundle `tests/fixtures/lsp-adapter/m23_lsp_bundle.json` + checked-in expected JSON/Markdown under `tests/fixtures/lsp-adapter/expected/` + byte-equality round-trip test `test_lsp_adapter_golden_round_trip` in `tests/unit/test_lsp_adapter.py` (14 focused LSP tests). The expected JSON pins `diagnostic_count=3`, `error_count=2`, `warning_count=1`, `info_count=0`, `output_bytes=562`, sorted category_ref_summary, and one diagnostic per code with deterministic `message_digest` SHA-256 first-16-hex digests. No raw message text embedded.
Done: M23-LSP-ADAPTER-GATE — focused + full repo-level gate sweep confirmed `1017 passed` (combined focused `119 passed`, LSP focused `14 passed`, DARS panel `75 passed`, governance `1 passed`, traceability OK, secret scan `hit_count=0`, `git diff --check` clean); LSP adapter line closed at `local_fixture_advisory_complete`. QUEUE-REFILL-PREP identified `M23-ADVANCED-ADAPTER-GATE` as the next safe Ralph row.
Done: M23-ADVANCED-ADAPTER-GATE — consolidated the entire M23 advanced codebase adapter milestone closure into a single record. OSS adapter sub-line closed at `0a172d3` (full repo `999 passed`); LSP adapter sub-line closed at `3ca9b22` (full repo `1017 passed`); both adapters pinned by deterministic byte-equality golden round-trip tests. M23 advanced codebase adapter milestone fully closed at `local_fixture_advisory_complete`. Final QUEUE-REFILL-PREP: all remaining post-M23 candidates require fresh explicit user authorization.
Next: QUEUE-REFILL-PREP-STOP — the M23 milestone is fully closed and no further safe local Ralph row is available without explicit user authorization. Post-M23 candidates: optional `M23-ADAPTER-PORTFOLIO-INTEGRATION` (additive evidence-line for M22 portfolio, not requested by user at M23 authorization); live LSP server execution against a real `ruff` / `pyright` / `mypy` / `flake8` / `eslint` binary (operator install + human gate required); real upstream OSS clone, license-text capture, or license adjudication (out of M23 scope); live provider DARS execution (out of scope); M24 or new-milestone work (requires user authorization). Stop and ask the user which (if any) post-M23 candidate to open next.
Pending: nothing actionable without user authorization.
```

Next safe Ralph queue target:

```text
QUEUE-REFILL-PREP-STOP — the M23 advanced codebase adapter milestone is closed at `local_fixture_advisory_complete`. No further safe local Ralph row is available without explicit user authorization. Stop and report; ask the user which (if any) of the following post-M23 candidates to open next: (a) optional `M23-ADAPTER-PORTFOLIO-INTEGRATION` (local docs/control + fixture-local implementation; adds `M23_OSS_ADAPTER` and `M23_LSP_ADAPTER` evidence-line refs to the M22 portfolio by ref/count only; not requested by user at M23 authorization); (b) live LSP server execution against a real `ruff` / `pyright` / `mypy` / `flake8` / `eslint` binary (operator install + `human_approval_ref` required); (c) real upstream OSS clone / license-text capture / license adjudication (out of M23 scope; never authorized); (d) live provider DARS execution (out of scope; local DARS panel completion remains `local_fixture_localhost_controlled_advisory_complete`); (e) M24 or new-milestone work. Do not advance past this stop point without an explicit user decision.
```
