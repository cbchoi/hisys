...
The active authoritative `/rloo` queue is this `ralph.md` file. The current branch is `/home/cbchoi/workspaces/develop/repos/hisys` on `dars`. M22 codebase evidence portfolio milestone is closed at `local_fixture_advisory_complete` (`cd944cc`). The M23 approved-OSS comparison adapter line is now closed at `local_fixture_advisory_complete` (`9963ccc`): PREP (`70ae484`), pure builder/writer + 6 focused tests (`d610c53`), thin CLI + 4 focused CLI tests (`d6023b4`), deterministic golden byte-equality round-trip (`9963ccc`), and closure gate evidence (full repo `999 passed`, focused combined `101 passed`, DARS panel `55 passed`). Remaining M23 backlog within the existing authorization is the local LSP adapter, the optional portfolio integration row, and the advanced adapter closure gate. The next safe Ralph row is `M23-LSP-ADAPTER-PREP` — a docs/control PREP packet that records the local LSP adapter subprocess boundary (command allowlist, timeout, workspace-root restriction, output schema, and kill policy) before any product code spawns anything.
...
Next: M23-LSP-ADAPTER-RED-GREEN — implement the pure governed runner per docs/plans/m23-lsp-adapter-implementation-tasks.md Tasks 0..4. RED first: PYTHONPATH=src pytest tests/unit/test_lsp_adapter.py::test_run_lsp_adapter_aggregates_ruff_diagnostics -q must fail with ModuleNotFoundError. Then GREEN: add src/hisys/operations/lsp_adapter.py with the four Pydantic shapes, the five JSON parsers (no eval/exec/pickle/yaml.load), the safety validator, and the writer; the test uses monkey-patched subprocess.run so no real LSP server is invoked. Add supplemental regression tests pinning timeout, oversize-output truncation, allowlist enforcement, shell-metacharacter rejection, missing-human-approval rejection, workspace-root containment, output-format allowlist, and FileNotFoundError handling.
Pending after LSP RED-GREEN: M23-LSP-ADAPTER-CLI, M23-LSP-ADAPTER-GOLDEN, M23-LSP-ADAPTER-GATE, M23-ADAPTER-PORTFOLIO-INTEGRATION, M23-ADVANCED-ADAPTER-GATE.
```

Next safe Ralph queue target:

```text
M23-LSP-ADAPTER-RED-GREEN — implement the pure governed local LSP adapter following `docs/plans/m23-lsp-adapter-implementation-tasks.md` Tasks 0..4. The first RED is a focused pytest that constructs an `LspAdapterRequest` with a caller-supplied `command_allowlist=("ruff",)`, monkey-patches `subprocess.run` to return a canned ruff JSON payload from `tests/fixtures/lsp-adapter/ruff/canned_ruff_output.json`, calls `run_lsp_adapter(request=...)`, and asserts the report aggregates diagnostics by severity/category — failing with `ModuleNotFoundError: No module named 'hisys.operations.lsp_adapter'` before the module exists. GREEN adds `src/hisys/operations/lsp_adapter.py` exposing `LspAdapterCommand`, `LspAdapterRequest`, `LspAdapterDiagnostic` (with SHA-256 first-16-hex `message_digest` only — no raw text), `LspAdapterReport`, `run_lsp_adapter`, `write_lsp_adapter_report`, plus the five format-specific parsers (`_parse_pyright_json`, `_parse_mypy_json`, `_parse_ruff_json`, `_parse_flake8_json`, `_parse_eslint_json`) all using `json.loads` only. The runner calls `subprocess.run(argv, cwd=workspace_root, env={"PATH": os.environ.get("PATH", "")}, timeout=timeout_seconds, capture_output=True, check=False, shell=False)` and rejects bad inputs with deterministic `ValueError` codes (`lsp_command_not_in_allowlist`, `lsp_timeout_out_of_range`, `lsp_output_format_not_allowed`, `lsp_argv_shell_metacharacter`, `lsp_human_approval_required`, `lsp_workspace_root_outside_instance`, `lsp_command_not_found`). Add the supplemental regression tests pinning timeout, oversize-output truncation (4 MiB cap), command-not-in-allowlist rejection, shell-metacharacter rejection, missing-approval rejection, workspace-root-outside-instance rejection, output-format-not-allowed rejection, and FileNotFoundError → ValueError conversion. Update `docs/traceability/README.md`, `docs/milestone-bootstrap/profile.yaml` (next_safe_task → `M23-LSP-ADAPTER-CLI`), and `tests/unit/test_governance_docs_current_state.py`. Do not add the CLI in this row.
```
