...
| Active task | R4 mapped-subscription panel config prep: move panel composition into Hisys DARS config and keep R4 action/R5/R7/R8 blocked. |
...
The current request authorizes local repository edits, docs/control updates, fixture-local tests, fake/injected transport tests, validation, one already-completed R3 Codex subprocess prompt-mode advisory smoke, local commit, and normal `git push origin dars` after validation. It selects the mapped-subscription claim bridge, but it does **not** authorize any further live provider/model calls, credential lookup, standing unattended approval, release tag/package/upload/deploy/publication, external notification, mutation outside a controlled Hisys runtime root, or removal of `requires_human_review=true`.
...
Next safe task: `DARS-LIVE-RELEASE-R4-PANEL-MAPPED-SUBSCRIPTION-ACTION-DECISION-PACKET` — prepare the human-gated R4 action decision packet using the named panel config. Do not invoke Codex, raw provider APIs, credential lookup, R4 action, R5 ACTION, R7 release-candidate readiness, or R8 release execution unless a later packet records exact scoped approval for the specific action.
