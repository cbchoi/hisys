...
| Active task | R3 ACTION decision packet selected `mapped_subscription`; next safe task is R4 mapped-subscription panel prep, not R4 action/R5/R7/R8. |
...
Next safe task: `DARS-LIVE-RELEASE-R4-PANEL-MAPPED-SUBSCRIPTION-PREP` — prepare the mapped-subscription panel packet and claim boundary. Do not invoke Codex, raw provider APIs, credential lookup, R4 action, R5 ACTION, R7 release-candidate readiness, or R8 release execution unless a later packet records exact scoped approval for the specific action.
