...
Done: M23-LSP-ADAPTER-CLI — thin `hisys lsp-adapter --instance <root> --date <YYYYMMDD> --bundle <json> [--current-head-short <hash>]` argparse subparser + `_load_lsp_adapter_bundle` loader + `_cmd_lsp_adapter` dispatcher landed inside `src/hisys/cli/main.py`, plus 4 focused CLI tests in `tests/unit/test_domain_cli.py` (happy-path round-trip with monkey-patched subprocess.run + canned ruff JSON; missing-command rejection; bad-date rejection; command-not-in-allowlist rejection). The CLI re-imports the M23-RED-GREEN Pydantic models so it does not duplicate shape definitions; the pure runner re-validates every safety invariant.
Next: M23-LSP-ADAPTER-GOLDEN — author `docs/plans/m23-golden-lsp-adapter-implementation-tasks.md` (mirroring `docs/plans/m23-golden-oss-comparison-adapter-implementation-tasks.md`) and pin a deterministic byte-equality round-trip golden test. The golden fixture must use a checked-in canonical canned subprocess output blob (e.g. `tests/fixtures/lsp-adapter/ruff/canned_ruff_output.json`), monkey-patch `subprocess.run` to return that blob, and compare the writer's JSON+Markdown output against checked-in expected files under `tests/fixtures/lsp-adapter/expected/`. The fixtures must not embed raw upstream source bodies, raw diagnostic messages, or any secret; they only pin severities, codes, line/column refs, message digests, and counts.
Pending after LSP GOLDEN: M23-LSP-ADAPTER-GATE, M23-ADAPTER-PORTFOLIO-INTEGRATION, M23-ADVANCED-ADAPTER-GATE.
```

Next safe Ralph queue target:

```text
M23-LSP-ADAPTER-GOLDEN — author `docs/plans/m23-golden-lsp-adapter-implementation-tasks.md` and add a deterministic byte-equality round-trip test in `tests/unit/test_lsp_adapter.py`. The golden bundle is a small JSON file under `tests/fixtures/lsp-adapter/expected/` that captures the canonical local-line + canned subprocess output configuration (date, current_head_short, command_id, workspace_root_ref, target_refs, command_allowlist, human_approval_ref). The test monkey-patches `subprocess.run` to return the existing canned ruff JSON, runs the pure runner, and compares the writer's persisted JSON + Markdown output against checked-in expected files. The expected JSON is the output of `model_dump(mode="json")` serialized with `json.dumps(..., indent=2, sort_keys=True) + "\n"` (matching the writer chokepoint); the expected Markdown is the output of `render_lsp_adapter_markdown(report)`. The fixtures do not regenerate at test time, do not depend on system clock / git hash / tempfile UUID, and do not embed raw upstream source bodies, raw diagnostic messages, or any secret. Update `docs/traceability/README.md`, `docs/milestone-bootstrap/profile.yaml` (next_safe_task → `M23-LSP-ADAPTER-GATE`), and `tests/unit/test_governance_docs_current_state.py`. Do not add the closure gate, portfolio integration row, or live LSP execution in this row.
```
