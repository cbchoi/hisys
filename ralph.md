...
The active authoritative `/rloo` queue is this `ralph.md` file. The current branch is `/home/cbchoi/workspaces/develop/repos/hisys` on `dars`. M22 codebase evidence portfolio milestone is closed at `local_fixture_advisory_complete` (`cd944cc`). The M23 approved-OSS comparison adapter line is closed at `local_fixture_advisory_complete` (`9963ccc`). The M23 local LSP adapter pure-runner row landed `src/hisys/operations/lsp_adapter.py` plus 13 focused safety tests in `tests/unit/test_lsp_adapter.py` under the governed subprocess boundary (caller-supplied command allowlist; 120s timeout cap; 4 MiB output truncation; workspace-root containment under `instance_root`; no shell; `env={"PATH": PATH}`; SHA-256 first-16-hex `message_digest` only — raw text never persisted; `FileNotFoundError` → `ValueError("lsp_command_not_found")`; `TimeoutExpired` → report with `subprocess_timed_out=true`, `diagnostics=()`). Remaining M23 backlog within the existing authorization is the LSP CLI wrapper, the LSP golden fixture, the LSP closure gate, the optional portfolio integration row, and the advanced adapter closure gate. The next safe Ralph row is `M23-LSP-ADAPTER-CLI` — a docs/control PREP packet that authors `docs/plans/m23-cli-lsp-adapter-implementation-tasks.md` and (in the next iteration) the thin `hisys lsp-adapter` argparse subparser following the M23-OSS-ADAPTER-CLI pattern.
...
Done: M23-LSP-ADAPTER-RED-GREEN — pure governed `src/hisys/operations/lsp_adapter.py` landed exposing `LspAdapterCommand`, `LspAdapterRequest`, `LspAdapterDiagnostic`, `LspAdapterReport`, `_validate_lsp_request`, five format-specific parsers (`_parse_ruff_json`, `_parse_pyright_json`, `_parse_mypy_json`, `_parse_flake8_json`, `_parse_eslint_json` — all `json.loads` only), `run_lsp_adapter`, `render_lsp_adapter_markdown`, and `write_lsp_adapter_report`. 13 focused tests in `tests/unit/test_lsp_adapter.py` pin the safety contract (ruff aggregation with canned JSON, allowlist rejection, timeout, oversize-truncation, output-format allowlist, timeout-range, shell-metacharacter, missing-approval, workspace-root containment, FileNotFoundError → lsp_command_not_found, unsafe target_refs collection, writer round-trip with no raw text in Markdown, writer bad-date rejection). No real LSP server is invoked; `subprocess.run` is monkey-patched in every test.
Next: M23-LSP-ADAPTER-CLI — author `docs/plans/m23-cli-lsp-adapter-implementation-tasks.md` following the M23-OSS-ADAPTER-CLI PREP pattern, then implement a thin `hisys lsp-adapter --instance <root> --date <YYYYMMDD> --request-bundle <json> [--current-head-short <hash>]` argparse subparser. The bundle JSON contains `workspace_root`, `command` (object with command_id/argv/timeout_seconds/expected_exit_codes/output_format), `target_refs`, `command_allowlist`, and `human_approval_ref`. Reuse the existing `_load_json_report` helper. Build the request through `LspAdapterCommand`/`LspAdapterRequest` Pydantic models so the CLI does not duplicate the M23 shape. Print bounded summary lines: `lsp-adapter report`, `markdown`, `command_id`, `diagnostic_count`, `error_count`, `warning_count`, `info_count`, `subprocess_exit_code`, `subprocess_timed_out`, `subprocess_killed`, `output_truncated`, `output_bytes`, `unsafe_ref_count`, `advisory_only: true`, `requires_human_review: true`, `external_call_made: false`, `mutation_performed: false`, `raw_source_content_persisted: false`, `live_external_action_authorized: false`, `allowed_actions: advisory_only`. The CLI must not call `date.today()`, must not read `.git/`, must not crawl `tests/fixtures/`, must not auto-discover bundle files, must not install LSP servers, must not invoke `pip`/`npm`/`apt-get`, and must not expand the command-id or output-format vocabulary.
Pending after LSP CLI: M23-LSP-ADAPTER-GOLDEN, M23-LSP-ADAPTER-GATE, M23-ADAPTER-PORTFOLIO-INTEGRATION, M23-ADVANCED-ADAPTER-GATE.
```

Next safe Ralph queue target:

```text
M23-LSP-ADAPTER-CLI — author `docs/plans/m23-cli-lsp-adapter-implementation-tasks.md` (mirroring `docs/plans/m23-cli-oss-comparison-adapter-implementation-tasks.md`) and implement a thin `hisys lsp-adapter --instance <root> --date <YYYYMMDD> --request-bundle <json> [--current-head-short <hash>]` subparser + dispatcher inside `src/hisys/cli/main.py`. The bundle JSON is a top-level object with `workspace_root` (str path), `command` (object with `command_id`, `argv` list, `timeout_seconds`, optional `expected_exit_codes`, `output_format`), `target_refs` (list of str), `command_allowlist` (list of str), and `human_approval_ref` (str). Reuse `_load_json_report` to load the bundle; construct the request through `LspAdapterCommand` / `LspAdapterRequest`. The CLI calls `run_lsp_adapter` and `write_lsp_adapter_report` without reshaping report semantics. RED-first: add a failing focused test in `tests/unit/test_domain_cli.py` that confirms the new subparser is not yet registered (argparse rejects `lsp-adapter`). GREEN: register the subparser, add the dispatcher, and the CLI invokes the pure runner with a monkey-patched `subprocess.run` returning a small canned ruff JSON payload. Regression tests pin: bundle-missing rejection, bad-date rejection, bad-command-id rejection, bad-bundle-shape rejection, and the bounded summary keys. Do not add the golden fixture, the closure gate, or live LSP execution in this row. Update `docs/traceability/README.md`, `docs/milestone-bootstrap/profile.yaml` (next_safe_task → `M23-LSP-ADAPTER-GOLDEN`), and `tests/unit/test_governance_docs_current_state.py`.
```
