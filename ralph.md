...
The active authoritative `/rloo` queue is this `ralph.md` file. The current branch is `/home/cbchoi/workspaces/develop/repos/hisys` on `dars`. M22 codebase evidence portfolio milestone is closed at `local_fixture_advisory_complete` (`cd944cc`). The user has explicitly authorized M23 advanced codebase adapter integration, including approved OSS comparison adapter work and optional local LSP adapter work; the OSS comparison adapter is opened first under the governed M23 boundary recorded below. The `M23-OSS-ADAPTER-PREP` row has authored the implementation-task plan at `docs/plans/m23-oss-comparison-adapter-implementation-tasks.md`, so the next safe Ralph row is the pure-builder RED/GREEN implementation.
...
Done: M23-OSS-ADAPTER-PREP — docs/plans/m23-oss-comparison-adapter-implementation-tasks.md authored with RED command, GREEN shape, regression cases, gate ritual, and CLI/GOLDEN/GATE deferral notes.
Next: M23-OSS-ADAPTER-RED-GREEN — write the failing `tests/unit/test_oss_comparison_adapter.py` first and only then add the pure module `src/hisys/operations/oss_comparison_adapter.py` and writer to satisfy it. Then add the supplemental regression tests pinned in the PREP packet.
Pending after OSS RED/GREEN: M23-OSS-ADAPTER-CLI, M23-OSS-ADAPTER-GOLDEN, M23-OSS-ADAPTER-GATE, M23-LSP-ADAPTER-PREP, M23-LSP-ADAPTER-RED-GREEN, M23-ADAPTER-PORTFOLIO-INTEGRATION, M23-ADVANCED-ADAPTER-GATE.
```

Next safe Ralph queue target:

```text
M23-OSS-ADAPTER-RED-GREEN — implement the pure approved OSS comparison adapter following `docs/plans/m23-oss-comparison-adapter-implementation-tasks.md` Tasks 0..4. RED first: `PYTHONPATH=src pytest tests/unit/test_oss_comparison_adapter.py::test_build_oss_comparison_aggregates_local_line_and_approved_sources -q` must fail with `ModuleNotFoundError`. Then GREEN: add `src/hisys/operations/oss_comparison_adapter.py` exposing `ApprovedOssSource`, `LocalCodebaseLine`, `OssComparisonRequest`, `OssComparisonReport`, `build_oss_comparison_report`, `render_oss_comparison_markdown`, and `write_oss_comparison_report` (writing under `runtime-boundary/oss-comparison/<YYYYMMDD>/comparison-report.{json,md}` via `resolve_instance_runtime_ref`). Add the supplemental regression tests (writer round-trip, unsafe ref/source-id/line-label/notes rejection, bad-date rejection). Update `docs/traceability/README.md`, `docs/milestone-bootstrap/profile.yaml` (next_safe_task → `M23-OSS-ADAPTER-CLI`), and `tests/unit/test_governance_docs_current_state.py`. Do not add the CLI in this row.
```
