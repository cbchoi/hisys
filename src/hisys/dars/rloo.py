"""DARS subsystem-local RLOO entry point.

This module makes the DARS subsystem individually executable without
depending on root-level Hisys RLOO orchestration. It exposes a
deterministic, side-effect-free readiness/status command:

    PYTHONPATH=src:. python3 -m hisys.dars.rloo --check --format json

The command reads ``src/hisys/dars/ralph.md``, composes the DARS-only
manifest and invocation-mode seams from :mod:`hisys.dars`, and emits a
machine-readable packet that pins the DARS authority locks recorded in
the controller and in
``docs/design/hisys-subsystem-architecture.md``.

The command does **not** call live providers, make raw provider API
calls, perform network requests, look up credentials, mutate vault or
evidence stores, push to remotes, or invoke other Hisys subsystems.
"""
...
_CONTROLLER_PATH = Path("src/hisys/dars/ralph.md")
...
def build_dars_subsystem_readiness_packet() -> dict[str, Any]:
    """Return a deterministic DARS subsystem readiness packet.

    The packet records the DARS subsystem identity, the controller anchor
    metadata, the DARS authority locks, the subsystem manifest, the
    documented invocation modes, and explicit side-effect/independence
    declarations. It is composed without any live external action.
    """
    ...
    manifest = asdict(get_dars_subsystem_manifest())
    invocation_modes = [asdict(mode) for mode in get_dars_subsystem_invocation_modes()]
    ...
    ready = (
        ...
        and metadata.get("subsystem") == "dars"
        and metadata.get("scope") == "DARS only"
        and current_task != ""
    )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="hisys.dars.rloo",
        description=(
            "DARS subsystem-local RLOO entry point. Reads the DARS RLOO "
            "controller and emits a deterministic, side-effect-free "
            "readiness packet so DARS can be exercised independently of "
            "root-level Hisys orchestration."
        ),
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help=(
            "Perform a DARS-only readiness check against "
            "src/hisys/dars/ralph.md without any live external action."
        ),
    )
    parser.add_argument(
        "--format",
